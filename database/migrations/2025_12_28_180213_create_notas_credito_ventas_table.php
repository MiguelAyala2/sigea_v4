...
return new class extends Migration
{
    ...
    {
        Schema::create('ventas.NOTAS_CREDITO', function (Blueprint $table) {
            $table->id();

            // Número de nota
            $table->string('numero_nota', 20)->unique()->comment('Número de nota de crédito generado');

            // Relaciones
            $table->unsignedBigInteger('factura_id')->comment('Factura a la que aplica la nota de crédito');
            ...
            // Fechas
            $table->date('fecha_emision')->comment('Fecha de emisión de la nota de crédito');

            // Motivo
            $table->text('motivo')->comment('Motivo de la nota de crédito');
            ...
            $table->decimal('total', 15, 2)->default(0)->comment('Total de la nota de crédito');

            // Facturación electrónica
            $table->boolean('es_electronica')->default(false)->comment('Si es nota de crédito electrónica');
            ...
            // Auditoría
            $table->unsignedBigInteger('creado_por')->nullable()->comment('Usuario que creó la NC');
            $table->unsignedBigInteger('actualizado_por')->nullable()->comment('Usuario que actualizó la NC');
            $table->unsignedBigInteger('emitido_por')->nullable()->comment('Usuario que emitió la NC');
            $table->timestamp('emitido_en')->nullable()->comment('Fecha y hora de emisión');
            $table->unsignedBigInteger('anulado_por')->nullable()->comment('Usuario que anuló la NC');
    ...
    {
        Schema::dropIfExists('ventas.NOTAS_CREDITO');
    }
};
