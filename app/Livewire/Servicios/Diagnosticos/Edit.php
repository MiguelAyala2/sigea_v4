...
class Edit extends Component
{
    ...
    public function mount($id)
    {
        ...
        $this->numero_diagnostico = $diagnostico->numero_diagnostico;
        $this->fecha_diagnostico = $diagnostico->fecha_diagnostico->format('Y-m-d');
        $this->cliente_id = $diagnostico->cliente_id;
        $this->solicitud_id = $diagnostico->solicitud_id;
        $this->recepcion_id = $diagnostico->recepcion_id;
        $this->producto_id = $diagnostico->producto_id;
        $this->problema_detectado = $diagnostico->problema_detectado;
        $this->solucion_propuesta = $diagnostico->solucion_propuesta;
        $this->estado_diagnostico = $diagnostico->estado_diagnostico;
        $this->estado = $diagnostico->estado;
        $this->observaciones = $diagnostico->observaciones;

        // Pre-cargar cliente para búsqueda
        $this->buscarCliente = $diagnostico->cliente->nombre;

        // Cargar solicitudes y recepciones del cliente
        $this->cargarSolicitudesYRecepciones();

        // Cargar solicitud y recepción seleccionadas
        $this->solicitudSeleccionada = $diagnostico->solicitud;
        $this->recepcionSeleccionada = $diagnostico->recepcion;

        // Cargar repuestos existentes
        foreach ($diagnostico->repuestos as $repuesto) {
        ...
        // Cargar tipos de servicio existentes
        foreach ($diagnostico->tiposServicio as $tipoServicio) {
            $tipo = $tipoServicio->tipoServicio;
            $this->tiposServicio[] = [
                'id' => $tipoServicio->id,
    ...
    public function cargarSolicitudesYRecepciones()
    {
        if ($this->cliente_id) {
            ...
            // Si no hay solicitud seleccionada, resetear
    ...
    public function agregarRepuesto($productoId)
    {
        ...
        if ($producto) {
            ...
            if (!$existe) {
                $precioUnitario = $producto->precioActual->precio_venta ?? 0;
    ...
    public function actualizar()
    {
        ...
        try {
            ...
            $diagnostico = Diagnostico::findOrFail($this->diagnosticoId);
            ...
            // Actualizar repuestos - eliminar los existentes y crear nuevos
            DiagnosticoRepuesto::where('diagnostico_id', $diagnostico->id)->delete();
            ...
            // Actualizar tipos de servicio - eliminar los existentes y crear nuevos
            DiagnosticoTipoServicio::where('diagnostico_id', $diagnostico->id)->delete();
            ...
            DB::rollBack();
            session()->flash('error', 'Error al actualizar el diagnóstico: ' . $e->getMessage());
        }
    }

    public function render()
    {
        return view('livewire.servicios.diagnosticos.edit');
    }
}
