...
class OrdenesCompraExport implements FromCollection, WithHeadings, WithMapping, WithStyles, WithTitle, WithEvents
{
    public function collection()
    {
        return OrdenCompra::with(['proveedor', 'pedidoCompra', 'creador'])
    ...
    {
        $condicion = match($orden->condicion_pago) {
            'CONTADO' => 'Contado',
            '7_DIAS' => '7 Días',
            '15_DIAS' => '15 Días',
            '30_DIAS' => '30 Días',
        ...
        $tipo = match($orden->tipo_orden) {
            'NORMAL' => 'Normal',
            'URGENTE' => 'Urgente',
            'SERVICIO' => 'Servicio',
            default => $orden->tipo_orden
        };

        return [
            $orden->numero_orden,
            $orden->fecha_orden ? $orden->fecha_orden->format('d/m/Y') : '-',
            $orden->proveedor->razon_social ?? 'N/A',
            $condicion,
            $tipo,
            $orden->total ?? 0,
            ucfirst(strtolower($orden->estado)),
            $orden->pedidoCompra->numero_pedido ?? '-',
        ];
    }
    ...
    public function headings(): array
    {
        ...
        return [
            ['LISTA DE ÓRDENES DE COMPRA - ' . $empresaNombre],
            ['Generado: ' . now()->format('d/m/Y H:i')],
            [],
            ['N° Orden', 'Fecha', 'Proveedor', 'Condición Pago', 'Tipo', 'Total (Gs.)', 'Estado', 'N° Pedido']
        ];
    }
