<div>
    ...
    <x-adminlte-card theme="primary" title="Datos de Diagnóstico" icon="fas fa-stethoscope" collapsible>
        <form wire:submit.prevent="guardar">
            <div class="row">
                <div class="col-md-3">
                    <x-adminlte-input name="numero_diagnostico" label="N° Diagnóstico"
                        value="{{ App\Models\Servicios\Diagnostico::generarNumeroDiagnostico() }}" disabled>
                ...
                <table class="table table-bordered table-sm">
                    ...
                    <tbody>
                        @foreach($repuestos as $index => $repuesto)
                            <tr>
                                <td><strong>{{ $repuesto['codigo'] ?? 'N/A' }}</strong></td>
                                ...
                                <td class="text-right">
                                    <input type="number" class="form-control form-control-sm text-right @if($repuesto['costo_unitario'] == 0) border-warning @endif"
                                           wire:model.live="repuestos.{{ $index }}.costo_unitario" min="0" step="0.01"
                                           placeholder="Ingrese costo"
                                           title="@if($repuesto['costo_unitario'] == 0)Sin precio configurado - Ingrese manualmente@else{{ number_format($repuesto['costo_unitario'], 0, ',', '.') }}@endif">
                                </td>
                                <td class="text-right">
    ...
    <div class="row">
        <div class="col-md-12">
            <button type="button" wire:click="guardar" class="btn btn-primary">
                <i class="fas fa-save"></i> Registrar Diagnóstico
            </button>
